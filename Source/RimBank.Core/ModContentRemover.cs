    ...
    internal class ModContentRemover
    {
        ...
        internal static int RemoveAllModContentsFromWorldObjects()
        {
            ...
            foreach (Settlement settlement in Find.WorldObjects.Settlements)
            {
                if (settlement.trader != null && field.GetValue(settlement.trader) != null)
                {
                    ...
                    {
                        if (settlement.trader.StockListForReading[num2].def == BankDefOf.BankNote || settlement.trader.StockListForReading[num2].GetInnerIfMinified().def == CoreDefOf.RimBankBuildingTerminal)
                        {
            ...
            foreach (Caravan caravan in Find.WorldObjects.Caravans)
            {
                foreach (Pawn item in caravan.PawnsListForReading)
                {
                    if (item.inventory != null && item.inventory.innerContainer.Count > 0)
                    {
                        ...
                        {
                            if (item.inventory.innerContainer[num3].def == BankDefOf.BankNote || item.inventory.innerContainer[num3].GetInnerIfMinified().def == CoreDefOf.RimBankBuildingTerminal)
                            {
            ...
            foreach (Map map in Find.Maps)
            {
                foreach (PassingShip passingShip in map.passingShipManager.passingShips)
                {
                    if (passingShip is TradeShip tradeShip)
                    {
                        ...
                        {
                            if (thingOwner[num2].def == BankDefOf.BankNote || thingOwner[num2].GetInnerIfMinified().def == CoreDefOf.RimBankBuildingTerminal)
                            {
        ...
        internal static int RemoveAllModContentsFromMaps()
        {
            ...
            foreach (Map map in Find.Maps)
            {
                ...
                {
                    if (map.spawnedThings[num2].def == BankDefOf.BankNote || map.spawnedThings[num2].GetInnerIfMinified().def == CoreDefOf.RimBankBuildingTerminal)
                    {
        ...
        internal static int RemoveAllModContentsFromAllPawns()
        {
            ...
            foreach (Pawn item in Find.WorldPawns.AllPawnsAliveOrDead)
            {
                if (item.inventory != null && item.inventory.innerContainer.Any)
                {
                    ...
                    {
                        if (item.inventory.innerContainer[num2].def == BankDefOf.BankNote || item.inventory.innerContainer[num2].GetInnerIfMinified().def == CoreDefOf.RimBankBuildingTerminal)
                        {
            ...
            foreach (Map map in Find.Maps)
            {
                foreach (Pawn allPawn in map.mapPawns.AllPawns)
                {
                    if (allPawn.inventory != null && allPawn.inventory.innerContainer.Any)
                    {
                        ...
                        {
                            if (allPawn.inventory.innerContainer[num3].def == BankDefOf.BankNote || allPawn.inventory.innerContainer[num3].GetInnerIfMinified().def == CoreDefOf.RimBankBuildingTerminal)
                            {
